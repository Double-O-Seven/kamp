...
internal class OnPlayerSelectPlayerMapObjectHandlerTest {
    ...
    fun shouldCallAllListeners() {
        ...
        onPlayerSelectPlayerMapObjectHandler.onPlayerSelectPlayerMapObject(playerMapObject, modelId, coordinates)

        verify(exactly = 1) {
            listener1.onPlayerSelectPlayerMapObject(playerMapObject, modelId, coordinates)
            listener2.onPlayerSelectPlayerMapObject(playerMapObject, modelId, coordinates)
            listener3.onPlayerSelectPlayerMapObject(playerMapObject, modelId, coordinates)
        }
    }
