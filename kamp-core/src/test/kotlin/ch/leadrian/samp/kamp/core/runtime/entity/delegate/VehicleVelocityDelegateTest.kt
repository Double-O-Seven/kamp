...
internal class VehicleVelocityDelegateTest {
    ...
    private lateinit var vehicleVelocityDelegate: VehicleVelocityDelegate

    @BeforeEach
    fun setUp() {
        every { vehicle.id } returns vehicleId
        vehicleVelocityDelegate = VehicleVelocityDelegate(nativeFunctionExecutor)
    }
    ...
    fun shouldGetVelocity() {
        ...
        val velocity = vehicleVelocityDelegate.getValue(vehicle, property)
    ...
    fun shouldSetVelocity() {
        ...
        vehicleVelocityDelegate.setValue(vehicle, property, vector3DOf(x = 1f, y = 2f, z = 3f))
