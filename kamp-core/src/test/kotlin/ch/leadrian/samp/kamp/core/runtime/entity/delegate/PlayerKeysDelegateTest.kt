...
internal class PlayerKeysDelegateTest {
    ...
    private lateinit var playerKeysDelegate: PlayerKeysDelegate

    @BeforeEach
    fun setUp() {
        every { player.id } returns playerId
        playerKeysDelegate = PlayerKeysDelegate(nativeFunctionExecutor)
    }
    ...
    fun shouldGetPlayerKeys() {
        ...
        val playerKeys = playerKeysDelegate.getValue(player, property)
