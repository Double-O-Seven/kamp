...
internal class PlayerVelocityDelegate(
    ...
    override fun getValue(thisRef: Player, property: KProperty<*>): Vector3D {
        nativeFunctionExecutor.getPlayerVelocity(playerid = thisRef.id.value, x = x, y = y, z = z)
        return vector3DOf(x = x.value, y = y.value, z = z.value)
    }

    override fun setValue(thisRef: Player, property: KProperty<*>, value: Vector3D) {
        nativeFunctionExecutor.setPlayerVelocity(playerid = thisRef.id.value, x = value.x, y = value.y, z = value.z)
    }
